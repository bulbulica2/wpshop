...
function wc_add_to_cart_message( $products, $show_qty = false, $return = false ) {
	...
	/* translators: %s: product name */
	$added_text = sprintf( _n( '%s a fost adaugat în coșul de cumpărături.', '%s au fost adaugate în coșul de cumpărături.', $count, 'woocommerce' ), wc_format_list_of_items( $titles ) );
	...
	if ( 'yes' === get_option( 'woocommerce_cart_redirect_after_add' ) ) {
		...
		$message = sprintf( '<a href="%s" tabindex="1" class="button wc-forward">%s</a> %s', esc_url( wc_get_cart_url() ), esc_html__( 'Vizualizează coșul', 'woocommerce' ), esc_html( $added_text ) );
	}
	...
	foreach ( $packages as $i => $package ) {
		...
		wc_get_template(
			...
			array(
				...
				'package_name'             => apply_filters( 'woocommerce_shipping_package_name', ( ( $i + 1 ) > 1 ) ? sprintf( _x( 'Shipping %d', 'shipping packages', 'woocommerce' ), ( $i + 1 ) ) : _x( 'Transport', 'shipping packages', 'woocommerce' ), $i, $package ),
				'index'                    => $i,
