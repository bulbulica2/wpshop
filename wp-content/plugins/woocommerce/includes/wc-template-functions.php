...
function wc_replace_policy_page_link_placeholders( $text ) {
	...
	$terms_page_id   = wc_terms_and_conditions_page_id();
	$privacy_link    = $privacy_page_id ? '<a href="' . esc_url( get_permalink( $privacy_page_id ) ) . '" class="woocommerce-privacy-policy-link" target="_blank">' . __( 'politica noastră de confidențialitate', 'woocommerce' ) . '</a>' : __( 'politica noastră de confidențialitate', 'woocommerce' );
...
if ( ! function_exists( 'woocommerce_template_loop_product_title' ) ) {
	...
	function woocommerce_template_loop_product_title() {
		echo '<h2 class="' . esc_attr( apply_filters( 'woocommerce_product_loop_title_classes', 'woocommerce-loop-product__title align_text_center' ) ) . '">' . get_the_title() . '</h2>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
}
...
if ( ! function_exists( 'woocommerce_catalog_ordering' ) ) {
	...
	function woocommerce_catalog_ordering() {
		...
		$catalog_orderby_options = apply_filters(
			...
			array(
				'menu_order' => __( 'Alege o altă sortare', 'woocommerce' ),
				'popularity' => __( 'Sortare după popularitate', 'woocommerce' ),
				'rating'     => __( 'Sortare după evaluarea medie', 'woocommerce' ),
				'date'       => __( 'Sortare după ultimul produs', 'woocommerce' ),
				'price'      => __( 'Sortare după preț: mic -> mare', 'woocommerce' ),
				'price-desc' => __( 'Sortare după preț: mare -> mic', 'woocommerce' ),
			)
		);
	...
	function woocommerce_checkout_payment() {
		...
		wc_get_template(
			...
			array(
				...
				'order_button_text'  => apply_filters( 'woocommerce_order_button_text', __( 'Plasează comanda', 'woocommerce' ) ),
			)
		);
	}
}
...
function wc_empty_cart_message() {
	echo '<p class="cart-empty woocommerce-info">' . wp_kses_post( apply_filters( 'wc_empty_cart_message', __( 'Coșul este gol momentan.', 'woocommerce' ) ) ) . '</p>';
}
